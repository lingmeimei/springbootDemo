...
------右键选中父级(modules)-->new-->module-->next-->
        goupId: 填写 父级(modules)所表示的goupId + artifactId；
        artifactId: 填写自己模块名;
       -->选择-->next-->
       Content root:填写 此父级(modules)所在的文件夹目录路径(包含此父级modules)\本模块名：
            "D:\ideaWorkSpace\test\springbootDemo\modules\cms-admin(或client-demo)" ;
